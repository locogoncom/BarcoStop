...
async function main() {
  ...
  const conversation = await postJson(`${base}/messages/conversation`, { userId1: user1?.id, userId2: user2?.id }, token);
  const avatar = await postAvatar(`${base}/users/${user1?.id}/avatar`, token);
